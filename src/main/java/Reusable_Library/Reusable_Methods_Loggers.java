...
public class Reusable_Methods_Loggers {
    ...
    public static WebDriver getDriver() throws InterruptedException {
        //kill all chrome driver instances before opening a new one
        //Thread.sleep(1000);
    ...
    //method to hover mouse on an element
    public static void mouseHover(WebDriver driver,String locator,String elementName){
        WebDriverWait wait = new WebDriverWait(driver,10);
        Actions actions = new Actions(driver);
        try{
            System.out.println("Mouse hovering value on element " + elementName);
            WebElement element = wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(locator)));
